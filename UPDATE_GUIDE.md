# v1.1 动效更新 · 现有 GitHub 仓库使用

本更新适用于之前的 Jiang_Qiao_Yan_GitHub_v1.0。无需新建仓库、重建 Pages、重新录入作品或更换依赖。

## 你需要做的步骤

1. 解压动效更新 ZIP。打开 GitHub Desktop 中的 `qiao_yan_1` → Show in Explorer。
...
4. 回到 GitHub Desktop，Summary 写 `Improve portfolio animations`，点击 Commit to main，然后 **Push origin**。
...
动效参数集中在 `src/scripts/motion.ts` 和 `src/styles/motion.css`；看图展开/收回在 `src/scripts/site.ts`。作品内容仍按 CONTENT_GUIDE.md 维护。没有新增第三方动画库、远程字体、外部脚本或图片预取。
...
检查记录见 `reports/MOTION_TEST_REPORT.md`。这是针对现有结构版的动效更新；真实画作设计、真机跨浏览器和线上网络验收仍按原已知事项处理。
