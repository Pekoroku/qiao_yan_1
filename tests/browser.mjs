...
for(const engine of engines){
 ...
 await check('detail deep refresh, story and focus viewer',async()=>{
  ...
  for(let i=0;i<10;i++){await page.locator('#open-viewer').click();assert(await page.locator('#art-viewer').evaluate(d=>d.open));await page.keyboard.press('Escape');assert.equal(await page.locator('body').evaluate(b=>b.classList.contains('viewer-open')),false);assert.equal(await page.evaluate(()=>document.activeElement.id),'open-viewer');}
